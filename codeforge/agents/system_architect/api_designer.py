            ...
            endpoints.extend(
                [
                    ...
                    APIEndpoint(
                        ...
                        request_schema={field.name: field.type for field in entity.fields if field.name != "id"},
                        response_schema={field.name: field.type for field in entity.fields},
                        auth_required=True,
                    ),
                    APIEndpoint("GET", f"/{resource}/{{id}}", f"Get {entity.name}", auth_required=True),
                    APIEndpoint("PUT", f"/{resource}/{{id}}", f"Update {entity.name}", auth_required=True),
                    APIEndpoint("DELETE", f"/{resource}/{{id}}", f"Delete {entity.name}", auth_required=True),
                ]
            )

        if any("export" in feature for feature in prd.scope.in_scope):
            endpoints.append(
                APIEndpoint("GET", "/export", "Export user data", response_schema={"csv": "string"}, auth_required=True)
            )
        return endpoints
